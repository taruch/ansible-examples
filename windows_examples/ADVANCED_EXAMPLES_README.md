...
This directory contains advanced Windows automation playbooks designed for enterprise-scale operations, system monitoring, and PowerShell integration.

## Table of Contents

1. [System Monitoring & Alert Response](#system-monitoring--alert-response)
2. [PowerShell Script Integration](#powershell-script-integration)
3. [Large-Scale Application Deployment](#large-scale-application-deployment)
...
### `system_monitoring_cpu_alert.yml`
...
ansible-playbook system_monitoring_cpu_alert.yml \
  -e "cpu_threshold=85" \
...
### `system_monitoring_memory_alert.yml`
...
**Example Execution**:
```bash
ansible-playbook system_monitoring_memory_alert.yml \
...
### `system_monitoring_disk_alert.yml`
...
**Example Execution**:
```bash
ansible-playbook system_monitoring_disk_alert.yml \
  -e "disk_threshold=90" \
...
```yaml
survey_spec:
  - question_name: "Share Name"
    variable: share_name
    type: text
    required: true
  - question_name: "Share Path"
    variable: share_path
    type: text
    required: true
  - question_name: "Change Users (comma-separated)"
    variable: change_users_input
    type: text
...
```bash
ansible-playbook mass_application_deployment.yml \
  -e "enable_rollback=false" \
  -e "backup_existing=false" \
  -e "_hosts=windows_dev"
```

**AAP Job Template Configuration**:
```yaml
name: "WINDOWS / Mass Application Deployment"
job_type: run
inventory: "Windows Production"
playbook: windows_examples/mass_application_deployment.yml
forks: 50  # Parallel execution within each batch
timeout: 7200  # 2 hours for large deployments
...
survey_spec:
  - question_name: "Target Hosts or Group"
    ...
    type: text
    required: true
    default: "windows_workstations"

  - question_name: "Application Installer URL"
  ...
  - question_name: "Batch Size (percentage)"
    variable: batch_size
    type: text
    default: "20%"
...
Each host reports:
- `deployed_version`: Version number installed
- `deployment_status`: `success` or `failed`
- `executable_verified`: Boolean validation result
- `backup_created`: Whether backup was created
- `rollback_attempted`: Whether rollback was triggered
- `rollback_successful`: Rollback outcome
...
- **Collections**:
  - `ansible.windows` (v2.0.0+)
  - `community.windows` (v2.0.0+)
- **WinRM Configuration**: HTTP (5985) or HTTPS (5986)
- **Credentials**: Domain or local admin account
...
To create job templates for these playbooks in AAP, run the root `setup_demo.yml`:

```bash
export CONTROLLER_PASSWORD=<password>
export CONTROLLER_USERNAME=<username>
export CONTROLLER_HOST=<hostname>
